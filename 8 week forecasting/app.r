...
#Outputs .CSV file with predicted attendances for the next 8 weeks (by hour).
...
server <- function(input, output) {
  ...
  output$dates <- renderUI({
    if(is.null(fcst_short())) {
      ...
      fluidRow(column(12,
                      wellPanel(sliderInput("dates", label = "Show dates:", min = mini, max = maxi, value = c(mini,maxi)))))
      }
  }
  )
  ...
  fcst_short <- eventReactive(input$go, {
    ...
    future <- make_future_dataframe(get(paste("m",x[i],sep="")),
                                    periods = (168 * 8) + if (24-hour(tail(df$ds,1)) == 23) {0} else {24-hour(tail(df$ds,1))},
                                    freq = 60*60, 
    ...
    fcst_short <- tail(combined, ((168 * 8)+if (24-hour(tail(df$ds,1)) == 23) {0} else {24-hour(tail(df$ds,1))}))
